...
class PoorviEngine(ExampleEngine):

    def pos_eval(position, engine):
      info = engine.analyse(position, chess.engine.Limit(time=1))
      return (chess.engine.PovScore.white(self=info["score"]))

    def search(self, board, *args):
        legal = list(board.legal_moves)
        url = "https://lichess.org/api/cloud-eval?fen=" + board.fen()
        r = requests.get(url)
        x = r.json()
        t = r.text
        ...
        return PlayResult(x["pvs"][0]["moves"][ 0 : 4 ], None)
